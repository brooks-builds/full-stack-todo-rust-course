...
| Yewdux Functional                  | x        | stores                    |                                                                                                                                       |                              |
| Persistent State                   | x        | stores                    | To handle staying logged in between page refreshes we can store our internal state, or at least the user information to local storage |                              |
| Installing Docker                  | x        | Setting Up                | A server and database has been created to help us focus on front end dev, we can use Docker to set that up easily                     |                              |
| Starting the Todo Server           | x        | Setting Up                | A crash course on using Docker and Docker-Dompose                                                                                     |                              |
| HTTP GET Requests                  | x        | http                      | Websites are boring unless we can get data from other locations                                                                       |                              |
| HTTP post requests                 | x        | http                      | We want to be able to create things in the database, this is usually done with a post request                                         |                              |
| Other HTTP requests                | x        | http                      | Updating resources uses either put or patch                                                                                           |                              |
